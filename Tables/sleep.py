...
def sevendays(showOperation, operation):
    result = sql.execute(
        f"SELECT * FROM sleep WHERE date <= {format.toDate(time.time())} AND date >= {format.toDate(time.time() - 7 * 86400)}", showOperation, "get")
    if len(result) > 0:
        sleepDurations = []
        morningTimes = []
        eveningTimes = []
        ...
        for row in result:
            color.printBlue(format.toHumanDate(str(row[0])) + " " + "Abends: " + str(
                row[1]).split(" ")[1] + ", " + "Morgens: " + str(row[2]).split(" ")[1])
        color.printMagenta(
...
def tobed(showOperation, operation):
    ...
    if time.localtime(time.time()).tm_hour < 10:
        ...
        f"SELECT title, priority FROM tasks WHERE status = 1 AND completion = '{currentTime.tm_year}-{currentTime.tm_mon}-{currentTime.tm_mday}' ORDER BY priority DESC", showOperation, "get")
    opentasks = sql.execute(
        f"SELECT title, priority FROM tasks WHERE status = 0 AND (workondate = '2020-12-12' OR workondate = '{currentTime.tm_year}-{currentTime.tm_mon}-{currentTime.tm_mday}') ORDER BY priority DESC", showOperation, "get")
    for row in done:
    ...
    for row in opentasks:
        ...
        s = input(f"")
    print("")
    result = getIntern("habits", ["habits_name", "habits_count", "habits_done"],
    ...
    for row in result:
        if str(row[2]) == "true":
            ...
            sql.execute(
                f"UPDATE habits SET habits_count = 0 WHERE habits_name = '{row[0]}'", showOperation, "post")
        i = i + 1
    sql.execute(f"UPDATE habits SET habits_done = 'false'",
                showOperation, "post")
    sql.execute(
        f"INSERT INTO sleep(date, evening) VALUES('{currentTime.tm_year}-{currentTime.tm_mon}-{currentTime.tm_mday}', '{format.currentDateTime()}')", showOperation, "post")
    color.printGreen("Good Night")


def frombed(showOperation, operation):
    sql.execute(
        f"UPDATE sleep SET morning = '{format.currentDateTime()}' WHERE morning IS NULL", showOperation, "post")
